...
def formatted_genre(genres: dict, top_n=5) -> str:
    ...
    return f"Top {top_n} generos mais jogados: " + ", ".join(partes)

def formatted_top5Games(games: dict, top_n=5) -> str:
    top_games = sorted(games.items(), key=lambda x: x[1], reverse=True)[:top_n]
    partes = [f"{nome}: {horas:.2f}h" for nome, horas in top_games]
...
def ai_responses(username, date_joined, total_games, time_played, time_played_per_genre, time_played_per_category, top_5, days_on_steam):
  return {
    ...
    "genreStats": genre_response(username, total_games, time_played_per_genre, top_5),
    "categoryStats": category_response(username, total_games, time_played_per_category, top_5)
  }